...
interface RohsDeclarationModalProps {
  products: Product[];
  onClose: () => void;
}

export function RohsDeclarationModal({ products, onClose }: RohsDeclarationModalProps) {
  const [formData, setFormData] = useState({
    ...
    declarationNumber: `ROHS-${Date.now()}`,
    logoUrl: '',
    signatureUrl: '',
  });
  ...
  const generateDeclarationLetter = async () => {
    ...
    addText('ROHS DECLARATION LETTER', 16, true, true);
    addSpacing(2);
    ...
    addText('ROHS DIRECTIVE 2011/65/EU DECLARATION', 14, true, true);
    addSpacing(2);
    
    addText('Dear Customer,', 10);
    addSpacing();
    
    addText(`We hereby declare that the following products supplied by ${formData.companyName} comply with the requirements of the RoHS Directive 2011/65/EU (Restriction of Hazardous Substances) and its amendments.`, 10);
    addSpacing(2);
    ...
    products.forEach((product, index) => {
      ...
      addText(`   RoHS Status: ${product.rohsStatus.toUpperCase()}`, 10);
    ...
    addText('1. The products listed above do not contain the following restricted substances in concentrations exceeding the maximum concentration values specified in Annex II of the RoHS Directive:', 10);
    addSpacing();
    ...
    const additionalStatements = [
      'All components and materials used in the manufacturing of these products have been tested and verified to comply with RoHS requirements.',
      'We maintain appropriate documentation and test reports to support this declaration.',
      'We continuously monitor our supply chain to ensure ongoing compliance with RoHS requirements.',
      'This declaration covers the products as manufactured and does not account for any modifications made after delivery.'
    ];
    
    additionalStatements.forEach((statement, index) => {
      addText(`${index + 2}. ${statement}`, 10);
      addSpacing();
    });
    ...
    addText('For any questions regarding this RoHS declaration, please contact:', 10);
    addSpacing();
    ...
    addText('This declaration is valid as of the date stated above and supersedes all previous RoHS declarations for the products listed.', 10);
    addSpacing(2);
    ...
    // Save the PDF
    pdf.save(`RoHS_Declaration_${formData.declarationNumber}.pdf`);
    
    alert('RoHS Declaration PDF has been generated and downloaded!');
  };
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full mx-4 max-h-screen overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold">Generate RoHS Declaration Letter</h2>
          </div>
